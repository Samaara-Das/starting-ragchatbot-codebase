...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Format: "Course Name - Lesson X|http://link"
            source = course_title
            if lesson_num is not None:
                source += f" - Lesson {lesson_num}"
            
            # Always append the link delimiter, even if link is None
...
class CourseOutlineTool(Tool):
    ...
    def execute(self, course_title: str) -> str:
        ...
        for course in all_courses:
            if course.get('course_title') == resolved_title:
    ...
    def _format_outline(self, course_metadata: Dict[str, Any]) -> str:
        ...
        course_title = course_metadata.get('course_title', 'Unknown Course')
        course_link = course_metadata.get('course_link', '')
        lessons = course_metadata.get('lessons', [])
        
        # Build formatted outline
        outline_parts = [f"Course Title: {course_title}"]
